...
<template>
  ...
  <section class="container px-4 mx-auto w-full">
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
          <div class="overflow-hidden border border-gray-200  md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 table-fixed">
              ...
              <tr>
                <th scope="col" class="hidden md:table-cell py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 ">
                  <div class="flex items-center gap-x-3">
                    <span>Id</span>
                  </div>
                </th>
                <th scope="col" class="hidden md:table-cell px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 ">
                  Data
                </th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="application in applications.data" :key="application.id">
                  <td class="hidden md:table-cell px-4 py-4 text-sm font-medium text-gray-700  whitespace-nowrap">
                    <div class="inline-flex items-center gap-x-3">
                      <span>#{{ application.id }}</span>
                    </div>
                  </td>
                  <td class="hidden md:table-cell px-4 py-4 text-sm text-gray-500  whitespace-nowrap">{{ application.created_at }}</td>
                  ...
                  <td class="px-4 py-4 text-sm whitespace-nowrap">
                    <div class="flex items-center gap-x-6 gap-y-1 flex-col md:flex-row md:w-36">
                      ...
                      <div v-if="application.status=='pendente'" class="flex gap-x-6 gap-y-1 flex-col md:flex-row">
                        <button @click="discard(application.id)"
                                class="text-red-500 transition-colors duration-200 underline underline-offset-4 hover:text-indigo-500 focus:outline-none">
                          Scarta
                        </button>

                        <button
                            class="text-green-600 transition-colors duration-200 underline underline-offset-4 hover:text-indigo-500 focus:outline-none"
    ...
    <div class="flex items-center justify-between mt-6">
      <Link :class="{'opacity-20':!applications.prev_page_url}"
            :href="applications.prev_page_url" preserve-scroll
      ...
      <div class="items-center hidden md:flex gap-x-3">
        <template v-for="(link,index) in applications.links">
          <Link preserve-scroll class="px-2 py-1 text-sm text-gray-500 rounded-md hover:bg-gray-100"
                v-if="link.url && index>0 && index<applications.links.length-1"
                :href="link.url" v-html="link.label" :class="link.active?'bg-blue-100/60':'hover:bg-gray-100'">
          </Link>
        </template>
      </div>

      <Link :class="{'opacity-20':!applications.next_page_url}" :href="applications.next_page_url" preserve-scroll
            class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100    ">
        <span>Next</span>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
             class="w-5 h-5 rtl:-scale-x-100">
